      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        >
          {featureKeys.map((_, i) => (
            ...
            >
              <div
                className={`group relative rounded-2xl border border-slate-200/80 bg-white p-7 transition-all hover:border-slate-300 hover:shadow-lg hover:shadow-slate-200/50 hover:-translate-y-0.5 h-full ${
                  i === 0 ? "lg:p-10" : ""
                }`}
              >
                <div
                  className={`mb-4 inline-flex items-center justify-center rounded-xl ${featureIcons[i].bg} p-2.5`}
                >
                  <div
                    className={`h-5 w-5 ${featureIcons[i].color}`}
...
function WhatsAppPreview() {
  const messages = [
    {
      from: "user",
      text: "Hola! Cuánto cuesta el tour a Machu Picchu para 4 personas?",
    },
    {
      from: "bot",
