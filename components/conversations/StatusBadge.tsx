...
const STATUS_CONFIG: Record<ConversationStatus, { label: string; color: string }> = {
  ...
  resolved: { label: "Resolved", color: "bg-slate-50 text-slate-500 border-slate-200" },
};
...
export default function StatusBadge({ status, botActive }: StatusBadgeProps) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className={`inline-flex h-5 items-center rounded-full border px-2 text-[10px] font-bold uppercase tracking-wider ${config.color}`}>
        {config.label}
      </span>
