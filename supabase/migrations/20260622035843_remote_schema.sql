...
COMMENT ON COLUMN "public"."contacts"."custom_attributes" IS 'Reemplaza Chatwoot custom_attributes. {agent: "activated"} pausa el bot para ese contacto.';
...
COMMENT ON COLUMN "public"."conversations"."bot_active" IS 'false cuando un agente tomó control. N8N verifica esto antes de responder.';



COMMENT ON COLUMN "public"."conversations"."assigned_agent_id" IS 'Usuario del portal que está atendiendo. Null = sin asignar.';
...
COMMENT ON COLUMN "public"."leads"."tour_interest" IS 'Tour o actividad que el cliente mencionó. Detectado por el bot.';



COMMENT ON COLUMN "public"."leads"."metadata" IS 'Contexto extraído por el bot. Ej: {dates: "marzo", group_size: 4, special_needs: ""}';
...
CREATE TABLE IF NOT EXISTS "public"."messages" (
    ...
    "channel" "text" DEFAULT 'whatsapp'::"text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "messages_role_check" CHECK (("role" = ANY (ARRAY['user'::"text", 'assistant'::"text", 'agent'::"text"])))
);
...
COMMENT ON COLUMN "public"."messages"."role" IS '"user" = cliente, "assistant" = bot, "agent" = agente humano del portal.';



COMMENT ON COLUMN "public"."messages"."from_bot" IS 'Permite filtrar en el dashboard: mensajes del bot vs mensajes de agentes humanos.';
...
COMMENT ON COLUMN "public"."organizations"."prompt" IS 'System prompt del bot. N8N lo consulta por org_id en cada mensaje entrante.';



COMMENT ON COLUMN "public"."organizations"."faqs" IS 'FAQs estructuradas para RAG. Formato: [{question: "", answer: ""}]';



COMMENT ON COLUMN "public"."organizations"."bot_config" IS 'Config operacional del bot. Ej: {buffer_seconds: 15, default_lang: "es"}';



COMMENT ON COLUMN "public"."organizations"."tours" IS 'Tours estructurados para edicion/UI. Formato [{id,name,category?,info,source?,confidence?}]. info=texto libre (precios/condiciones). El prompt se compila desde aqui + faqs + personalidad.';



COMMENT ON COLUMN "public"."organizations"."business_info" IS 'Contexto general del negocio en secciones tituladas. Formato [{id,title,content,source?,confidence?}]. Ej: Sobre nosotros, Politicas, Formas de pago, Contacto. Se compila al prompt junto a tours + faqs.';
...
COMMENT ON TABLE "public"."subscriptions" IS 'Sincronizada vía Stripe webhooks. Si status != active, el bot se pausa.';
...
COMMENT ON COLUMN "public"."users"."role" IS 'admin: puede editar config del bot. agent: solo atiende conversaciones.';
...
COMMENT ON TABLE "public"."whatsapp_accounts" IS 'Cuentas de WhatsApp conectadas via Meta Embedded Signup. Una por org.';



COMMENT ON COLUMN "public"."whatsapp_accounts"."waba_id" IS 'WhatsApp Business Account ID de Meta.';



COMMENT ON COLUMN "public"."whatsapp_accounts"."phone_number_id" IS 'ID que Meta asigna al numero. Se usa para enviar mensajes y para identificar la org en el webhook.';



COMMENT ON COLUMN "public"."whatsapp_accounts"."phone_number" IS 'Numero en formato E.164. El numero del cliente.';



COMMENT ON COLUMN "public"."whatsapp_accounts"."access_token" IS 'Token para Graph API. Sensible — considerar encriptar con pgcrypto o mover a Vault.';
...
ALTER TABLE ONLY "public"."whatsapp_accounts"
    ADD CONSTRAINT "whatsapp_accounts_org_id_key" UNIQUE ("org_id");
...
CREATE INDEX "idx_contacts_org_phone" ON "public"."contacts" USING "btree" ("org_id", "phone");
...
CREATE INDEX "idx_conversations_bot_active" ON "public"."conversations" USING "btree" ("bot_active");
...
CREATE INDEX "idx_conversations_org_created" ON "public"."conversations" USING "btree" ("org_id", "created_at" DESC);
...
CREATE INDEX "idx_leads_org_created" ON "public"."leads" USING "btree" ("org_id", "created_at" DESC);
...
CREATE INDEX "idx_organizations_onboarded_at" ON "public"."organizations" USING "btree" ("onboarded_at");
...
CREATE INDEX "idx_whatsapp_accounts_org" ON "public"."whatsapp_accounts" USING "btree" ("org_id");
...
CREATE OR REPLACE TRIGGER "trg_whatsapp_accounts_updated_at" BEFORE UPDATE ON "public"."whatsapp_accounts" FOR EACH ROW EXECUTE FUNCTION "public"."set_updated_at"();
...
ALTER TABLE ONLY "public"."whatsapp_accounts"
    ADD CONSTRAINT "whatsapp_accounts_org_id_fkey" FOREIGN KEY ("org_id") REFERENCES "public"."organizations"("id");



CREATE POLICY "admins_delete_conversations" ON "public"."conversations" FOR DELETE TO "authenticated" USING ((("org_id" = "public"."auth_org_id"()) AND ("public"."auth_role"() = 'admin'::"text")));



CREATE POLICY "admins_delete_messages" ON "public"."messages" FOR DELETE TO "authenticated" USING ((("conversation_id" IN ( SELECT "conversations"."id"
   FROM "public"."conversations"
  WHERE ("conversations"."org_id" = "public"."auth_org_id"()))) AND ("public"."auth_role"() = 'admin'::"text")));
...
CREATE POLICY "contacts_insert_own_org" ON "public"."contacts" FOR INSERT WITH CHECK (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "contacts_select_own_org" ON "public"."contacts" FOR SELECT USING (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "contacts_update_own_org" ON "public"."contacts" FOR UPDATE USING (("org_id" = "public"."get_user_org_id"()));
...
CREATE POLICY "conversations_insert_own_org" ON "public"."conversations" FOR INSERT WITH CHECK (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "conversations_select_own_org" ON "public"."conversations" FOR SELECT USING (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "conversations_update_own_org" ON "public"."conversations" FOR UPDATE USING (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "deny_all_login_attempts" ON "public"."login_attempts" USING (false);
...
CREATE POLICY "leads_insert_own_org" ON "public"."leads" FOR INSERT WITH CHECK (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "leads_select_own_org" ON "public"."leads" FOR SELECT USING (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "leads_update_own_org" ON "public"."leads" FOR UPDATE USING (("org_id" = "public"."get_user_org_id"()));
...
CREATE POLICY "messages_insert_own_org" ON "public"."messages" FOR INSERT WITH CHECK (("conversation_id" IN ( SELECT "conversations"."id"
   FROM "public"."conversations"
  WHERE ("conversations"."org_id" = "public"."get_user_org_id"()))));



CREATE POLICY "messages_select_own_org" ON "public"."messages" FOR SELECT USING (("conversation_id" IN ( SELECT "conversations"."id"
   FROM "public"."conversations"
  WHERE ("conversations"."org_id" = "public"."get_user_org_id"()))));



CREATE POLICY "org_select_own" ON "public"."organizations" FOR SELECT USING (("id" = "public"."get_user_org_id"()));



CREATE POLICY "org_update_admin_only" ON "public"."organizations" FOR UPDATE USING ((("id" = "public"."get_user_org_id"()) AND (EXISTS ( SELECT 1
   FROM "public"."users"
...
CREATE POLICY "service role full access" ON "public"."embeddings" USING (true) WITH CHECK (true);



CREATE POLICY "subscription_select_own" ON "public"."subscriptions" FOR SELECT USING (("org_id" = "public"."get_user_org_id"()));
...
CREATE POLICY "users_select_own_or_org" ON "public"."users" FOR SELECT USING ((("id" = "auth"."uid"()) OR ("org_id" = "public"."get_user_org_id"())));
...
CREATE POLICY "whatsapp_insert_admin" ON "public"."whatsapp_accounts" FOR INSERT WITH CHECK ((("org_id" = "public"."get_user_org_id"()) AND (EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."role" = 'admin'::"public"."user_role"))))));



CREATE POLICY "whatsapp_select_own" ON "public"."whatsapp_accounts" FOR SELECT USING (("org_id" = "public"."get_user_org_id"()));



CREATE POLICY "whatsapp_update_admin" ON "public"."whatsapp_accounts" FOR UPDATE USING ((("org_id" = "public"."get_user_org_id"()) AND (EXISTS ( SELECT 1
   FROM "public"."users"
  WHERE (("users"."id" = "auth"."uid"()) AND ("users"."role" = 'admin'::"public"."user_role"))))));
